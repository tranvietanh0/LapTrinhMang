...
> **Cập nhật 27/09/2026:** (1) chướng ngại vật đứng yên đổi thành **xe cộ chạy cùng chiều**: `Obstacle` có thêm `speed`, `kind` và `positionAt(tick)`; 48 xe (16 xe mỗi làn), mỗi làn một tốc độ 110 / 160 / 210 km/h xáo theo seed; server xét va chạm tại vị trí đã di chuyển, choáng 1,5 s (S4, S5). (2) Luật chơi mới: đường đua 2500 m, tốc độ tối đa 360 km/h, **giữ** W để tăng tốc 120 km/h mỗi giây, giữ S phanh 300 km/h/s, nhả ga xe tự giảm 40 km/h/s (thay cho tăng / giảm theo từng lần nhấn). (3) Màn hình đua làm lại theo kiểu game đua xe cổ điển nhìn từ trên xuống (Road Fighter): camera cuộn, cảnh quan, xe pixel, vụ nổ, HUD đen, minimap (R1–R6; lớp mới `TrackView`, `PixelArt`). (4) `ClientHandler` gửi qua **hàng đợi riêng mỗi client** (256 tin) và luồng ghi riêng, client không nhận kịp bị ngắt thay vì làm đứng cả phòng (S6, S7). (5) Server **gửi lại `ONLINE_LIST`** cho người vừa đăng nhập ngay sau `LOGIN_RESULT` (sửa lỗi người vào sau thấy sảnh trống). (6) Server có tùy chọn **`--memory`** chạy không cần MySQL. Báo cáo `report/` đã cập nhật theo các thay đổi này.
...
4. Phòng đua: đếm ngược 3-2-1, hai đường đua song song, cùng dòng xe cộ chạy cùng chiều (chướng ngại vật), đồng bộ vị trí hai xe.
...
│       └── GameConfig.java      # PORT=5000, TRACK_LENGTH=2500, LANES=3, TICK_MS=50,
│                                # INVITE_TIMEOUT_S=30, DISCONNECT_TIMEOUT_S=15, MAX_SPEED=360
├── server/
...
│       └── ui/race/RaceFrame, RacePanel, TrackView, PixelArt, ResultDialog, RematchDialog, RaceDemo
...
| S4 | 2 | `Room` + `RoomManager`: tạo phòng, sinh 48 xe cộ theo seed bằng `ObstacleGenerator` (mỗi làn một tốc độ, giống nhau 2 bên), gửi `MATCH_START`, `COUNTDOWN` 3-2-1-0, tick 50 ms nhận `CAR_STATE`, cắt ngưỡng tốc độ / quãng đường, tính va chạm với xe cộ tại `positionAt(tick)` (`CarSim`), phát `RACE_UPDATE` | `core/Room`, `core/RoomManager`, `core/CarSim`, `core/ObstacleGenerator` | 2 client thấy xe nhau chạy; gửi tốc độ 999 bị cắt về 360; đâm xe cộ làm tốc độ về 0 trong 1,5 s, va chạm tính theo vị trí xe cộ đã di chuyển | 8 | S3, D2 |
| S5 | 3 | `MatchService`: xét về đích, hòa cùng tick, thoát, mất kết nối; gọi `MatchDAO.saveResult`; gửi `MATCH_RESULT`; luồng `REMATCH_ASK` / `REMATCH_REPLY`, chỉ tạo ván mới khi cả hai đồng ý | `core/MatchService` | Điểm trong DB đúng sau mỗi tình huống ở mục 5; rematch sinh lại dòng xe cộ và reset quãng đường | 6 | S4, D4 |
...
| Mã | Tuần | Việc | File / lớp | Tiêu chí hoàn thành | Giờ | Phụ thuộc |
|---|---|---|---|---|---|---|
| R1 | 1 | `RacePanel` + `TrackView` + `PixelArt`: chia đôi màn hình, mỗi bên một camera cuộn bám xe (kiểu Road Fighter), 3 làn, cỏ / cây / nhà / ao hai bên, vạch START / FINISH, xe pixel theo `kind`, HUD đen và minimap | `client/ui/race/RacePanel`, `TrackView`, `PixelArt` | Chạy `RaceDemo`, cửa sổ 1200×800 hiển thị đúng như Hình 6 báo cáo | 6 | D2 |
| R2 | 2 | `CarModel` + điều khiển: Key Bindings W/S/A/D và mũi tên, giữ W tăng tốc 120 km/h/s, giữ S phanh 300 km/h/s, nhả ga tự giảm 40 km/h/s; `Timer` 50 ms cập nhật quãng đường theo tốc độ, `Timer` 16 ms vẽ lại, HUD KM/H / quãng đường / minimap | `client/model/CarModel`, `RaceFrame`, `RacePanel` | Đường cuộn mượt, đổi làn trượt mượt, không vượt 360 km/h | 6 | R1 |
...
| R4 | 3 | Nhận `RACE_UPDATE`: hiệu chỉnh xe mình, cập nhật xe đối thủ, lưu `tick` để vẽ xe cộ bằng `positionAt` (nội suy giữa 2 lần cập nhật); va chạm hiện vụ nổ, xe cộ bị đâm biến mất; dừng `Timer` khi trận kết thúc | `RaceFrame`, `TrackView` | Hai máy thấy vị trí như nhau (lệch < 1 tick); va chạm hiện đúng lúc | 5 | R3 |
...
| R6 | 4 | Hiệu năng và tinh chỉnh: bộ đệm sprite dựng sẵn, chỉ vẽ vật trong khung nhìn, không tạo font / stroke mỗi khung hình, màu xe theo người chơi; sửa lỗi hiển thị từ kiểm thử | `client/ui/race/*` | CPU client < 15 % khi đua; không còn lỗi hiển thị | 4 | R5 |
...
| T8 | Đâm vào xe cộ | Tốc độ về 0 trong 1,5 s trên cả hai màn hình, xe cộ bị đâm nổ và biến mất | Việt Anh | Phương / Mai Anh |
...
| T13 | Cả hai đồng ý thi đấu tiếp | Ván mới trong cùng phòng, dòng xe cộ khác, quãng đường về 0 | Việt Anh | Phương / Mai Anh |
...
| T17 | Gửi `CAR_STATE` giả tốc độ 999 (test bằng client sửa) | Server cắt về 360, không thắng nhanh bất thường | Việt Anh | Phương |
