...
| T8 | Va chạm xe cộ | Đuổi kịp và lái xe đâm vào đuôi một xe cộ đang chạy cùng làn | Tốc độ về 0 trong 1,5 s, xe cộ bị đâm nổ và biến mất, hiệu ứng hiện trên cả hai màn hình cùng lúc | `match_events` có COLLISION | Việt Anh | | |
...
| T17 | Gửi CAR_STATE tốc độ 999 | Chạy `t17-speed-cheat.txt` với đối thủ thật | RACE_UPDATE trả về `speed ≤ 360` (MAX_SPEED); client gian lận không về đích nhanh hơn xe 360 km/h | `matches` không có kết quả bất thường | Việt Anh | | Đã chạy 24/09 (khi đó MAX_SPEED = 200, đường 1000 m) với server `--memory`: server tự tính quãng đường từ tốc độ đã cắt về 200, client gian lận ngừng gửi sau khi "tự về đích" nên xe đứng lại ở ~211 m và thua |
